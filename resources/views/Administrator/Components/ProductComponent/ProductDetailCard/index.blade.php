...
<section class="bg-gradient-to-r from-blue-50 to-gray-100 py-12 flex justify-center">
    <div class="bg-white rounded-3xl shadow-2xl max-w-5xl w-full p-8 flex flex-col md:flex-row items-center md:items-start gap-8 transition hover:shadow-3xl">
        ...
        <div class="w-full md:w-1/2">
            <img src="{{ asset($productData->product_image) }}"
        ...
        <div class="w-full md:w-1/2 flex flex-col gap-4">
            ...
            <div class="mt-4 flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6">
                <span class="text-3xl font-semibold text-gray-900">
                    ${{ number_format($productData->product_price, 2) }}
                </span>
                @if($productData->product_stock > 0)
                    <span class="px-4 py-1 bg-green-100 text-green-800 font-medium rounded-full">
                        In Stock
                    </span>
                @else
                    <span class="px-4 py-1 bg-red-100 text-red-800 font-medium rounded-full">
                        Out of Stock
                    </span>
                @endif
            </div>
            ...
            <div class="mt-6 flex flex-col sm:flex-row gap-4">
                <!-- Edit Button -->
                <a href="{{ route('Administrator.Products.edit', $productData->id) }}"
                   class="inline-block bg-blue-600 text-white py-3 px-8 rounded-xl font-medium hover:bg-blue-700 transition shadow-md hover:shadow-lg text-center">
                    Edit Product
                </a>

                <!-- Back Button -->
                <a href="{{ route('Administrator.Products.index') }}"
                   class="inline-block border border-gray-300 text-gray-700 py-3 px-8 rounded-xl font-medium hover:bg-gray-100 transition shadow-sm hover:shadow-md text-center">
                    Back to Products
                </a>
            </div>
        </div>
